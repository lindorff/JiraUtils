...
describe("Jira", () => {
    ...
    describe("getIssueStatusEvents", () => {
        it("should find a status item", () => {
            exampleIssue.changelog.histories[0].items = [historyItem({ field: "status", from: "a", to: "b" })];
            ...
            expect(item.from).to.equal("a");
            expect(item.to).to.equal("b");
        });

        it("should find many status items", () => {
            exampleIssue.changelog.histories[0].items = [
                historyItem({ field: "status", from: "a", to: "b" }),
                historyItem({ field: "status", from: "c", to: "d" })
            ];
            ...
            expect(item1.from).to.equal("a");
            expect(item1.to).to.equal("b");

            const item2 = statusEvents[0].items[1];
            expect(item2.from).to.equal("c");
            expect(item2.to).to.equal("d");
        });

        it("should discard any non-status items", () => {
            exampleIssue.changelog.histories[0].items = [historyItem({ field: "foo", from: "a", to: "b" })];
